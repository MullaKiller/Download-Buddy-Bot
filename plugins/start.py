...
from plugins.utils.utility import MemberTagger, random_emoji
...
async def edit_channel_messages_and_media(client: Bot, message: Message):
    try:
        ...
        await client.send_reaction(chat_id=message.chat.id, message_id=message.id, emoji=random_emoji())
    except Exception as e:
